document.addEventListener("DOMContentLoaded", function () {
  ...
  async function fetchRatings() {
    ...
    try {
      const response = await fetch(`/api/ratings?date=${dateInput.value}`);
      if (!response.ok) {
        throw new Error(`HTTP ${response.status}: ${response.statusText}`);
      }
      const data_1 = await response.json();
      console.log("Raw ratings data from backend:", data_1);
      ratings = data_1;
      return data_1;
    } catch (error) {
      ...
      if (error.message.includes("HTTP 429")) {
        console.warn(
          "Rate limited while fetching ratings, keeping existing data",
        );
  ...
  function renderStars(rating, foodId, isInteractive = true) {
    ...
    if (isInteractive) {
      const starRating = document
        .getElementById("star-rating-template")
      ...
      stars.forEach((star) => {
        ...
        star.addEventListener("click", (event) => {
          ...
          const current = userRatings[foodId]
            ? parseInt(userRatings[foodId], 10)
            : 0;
            ...
            fetch("/api/rate", {
              ...
              .then((data) => {
                ...
                fetchRatings()
                  ...
                  .catch((e) => {
                    console.warn(
                      "Failed to refresh ratings after submission:",
                      e,
                    );
                  });
              })
              .catch((error) => {
                ...
                if (error.message.includes("HTTP 429")) {
                  alert(
                    "Rate limit exceeded. Please wait a moment before rating more items.",
                  );
                } else {
  ...
  function filterRatingsByMenu(ratings, menuData) {
    ...
    for (const [foodKey, rating] of Object.entries(filteredRatings.foods)) {
      ...
      stationRatings[stationKey].total +=
        rating.avg_rating * rating.rating_count;
    ...
    for (const [foodKey, rating] of Object.entries(filteredRatings.foods)) {
      ...
      diningHallRatings[diningHall].total +=
        rating.avg_rating * rating.rating_count;
    ...
    fetch(`/api/menus?date=${date}`)
      ...
      .then((data) => {
        ...
        for (const diningHall in data) {
          ...
          const orderedMeals = ["breakfast", "lunch", "dinner"].filter((m) =>
            data[diningHall].hasOwnProperty(m),
          );
          for (const meal of orderedMeals) {
            ...
            if (
              openTabs.has(mealId) ||
              (currentMeal &&
                meal.toLowerCase() === currentMeal.toLowerCase() &&
                openTabs.size === 0)
            ) {
            ...
            if (
              data[diningHall][meal] &&
              Object.keys(data[diningHall][meal]).length > 0
            ) {
              for (const station in data[diningHall][meal]) {
                ...
                if (ratings.stations[`${station}_${diningHall}_${meal}`]) {
                  const avgRating =
                    ratings.stations[`${station}_${diningHall}_${meal}`]
                      .avg_rating;
                ...
                items.forEach((item) => {
                  ...
                  const communityObj = ratings.foods[foodRatingKey] || {
                    avg_rating: 0,
                    rating_count: 0,
                  };
                  const communityStars = renderStars(
                    communityObj.avg_rating,
                    null,
                    false,
                  );
                  ...
                  if (communityObj.dist) {
                    ...
                    const total =
                      Object.values(communityObj.dist).reduce(
                        (a, b) => a + b,
                        0,
                      ) || 1;
                    for (let i = 1; i <= 5; i++) {
                  ...
                  const myRating = userRatings[item.id]
                    ? parseInt(userRatings[item.id], 10)
                    : 0;
  ...
  function updateLiveAggregates(starContainer, newRating) {
    try {
      ...
      const station = li.closest(".station").querySelector("h4")
        .dataset.originalName;
      const diningHall = li.closest(".dining-hall").querySelector("h2")
        .dataset.originalName;
      const mealElement = li.closest(".meal");
      const mealTitle = mealElement.querySelector("h3").dataset.originalName;

      console.log(
        `Extracted: station=${station}, diningHall=${diningHall}, mealTitle=${mealTitle}`,
      );
      console.log(
        `Updating aggregates for: ${station} in ${diningHall} ${mealTitle} with rating ${newRating}`,
      );
  ...
  function updateCommunityRatingDisplay(starContainer, _newRating) {
    try {
      const li = starContainer.closest("li[data-food-id]");
      if (!li) return;
      ...
      const userRating = userRatings[foodId]
        ? parseInt(userRatings[foodId], 10)
        : 0;
      ...
      if (communityRating && communityRating.avg_rating > 0) {
        ...
        console.log(
          `Using server community data: ${trueAverage.toFixed(2)} stars, ${trueCount} ratings`,
        );
      ...
      console.log(
        `Community display update: ${foodName}, True average: ${trueAverage}, Count: ${trueCount}`,
      );
      ...
      if (communityRating && communityRating.dist) {
        ...
        const total =
          Object.values(communityRating.dist).reduce((a, b) => a + b, 0) || 1;
        for (let i = 1; i <= 5; i++) {
          const bar = document.createElement("div");
          bar.classList.add("bar");
  ...
  function calculateAverageForItems(items) {
    ...
    items.forEach((li) => {
      ...
      console.log(
        `Meal Title: ${mealTitle ? mealTitle.textContent : "null"}, Original Meal: ${mealSlug}`,
      );
      ...
      if (communityRating && communityRating.avg_rating > 0) {
        ...
        if (foodName === "Lemon Basil Salmon") {
          console.log(
            `🐟 Added community: ${communityRating.avg_rating} * ${communityRating.rating_count} = ${communityRating.avg_rating * communityRating.rating_count}`,
          );
        }
      } else if (userRating > 0) {
    ...
    console.log(
      `Calculated average: ${total}/${count} = ${average.toFixed(2)}`,
    );
    return average;
  }
  ...
  function updateStationRating(diningHall, station, specificMealPeriod) {
    ...
    console.log(
      `Looking for: ${station} in ${diningHall} ${specificMealPeriod}`,
    );
      ...
      if (hallTitle && hallTitle.dataset.originalName === diningHall) {
        hall.querySelectorAll(".meal").forEach((meal) => {
          ...
          const mealName = mealTitle
            ? mealTitle.dataset.originalName
            : "NO_TITLE";
          ...
          if (mealName === specificMealPeriod) {
            meal.querySelectorAll(".station").forEach((st) => {
              ...
              if (stTitle && stTitle.dataset.originalName === station) {
                ...
                const existing = stTitle.querySelector(
  ...
  function updateDiningHallRating(diningHall) {
    ...
    const diningHallAvg =
      mealAverages.length > 0
        ? mealAverages.reduce((sum, avg) => sum + avg, 0) / mealAverages.length
        : 0;

    console.log(
      `Calculated dining hall average from ${mealAverages.length} meal periods: ${diningHallAvg.toFixed(2)}`,
    );
  ...
  function updateAllAggregatesFromRatings() {
    console.log(
      "updateAllAggregatesFromRatings called - now using direct updates",
    );
  ...
  document.addEventListener("click", function (event) {
    ...
    if (target.tagName === "H2" && target.dataset.diningHallId) {
      const diningHallContent = document.getElementById(
        target.dataset.diningHallId,
      );
      const isActive = diningHallContent.classList.toggle("active");

      // Close sibling dining halls at same level
      const siblingHalls = document.querySelectorAll(
        ".dining-hall .dining-hall-content",
      );
    ...
    // Handle meal title clicks
      ...
      const diningHallContent = document.getElementById(
        target.dataset.diningHallContent,
      );
      const isActive = mealContent.classList.toggle("active");

      // Close sibling meals at same level
      const siblingMeals = diningHallContent.querySelectorAll(
        ".meal .meal-content",
      );
